...
var CharacterAction = function () {
	function CharacterAction() {
		_classCallCheck(this, CharacterAction);

		this.generateActions('reportSuccess', 'reportFail', 'getCharacterSuccess', 'getCharacterFail');
	}

	_createClass(CharacterAction, [{
		key: 'getCharacter',
		...
		value: function report(characterId) {
			...
			$.ajax({
				...
				_this2.action.reportFail(jqXHR);
			});
		}
	}]);
...
exports.default = _alt2.default.createActions(CharacterAction);
	...
	_createClass(FooterActions, [{
		...
		value: function getTopCharacters() {
			...
			$.ajax({ url: '/api/characters/top' }).done(function (data) {
				_this.action.getTopCharactersSuccess(data);
			}).fail(function (jqXHR) {
				_this.action.getTopCharactersFail(jqXHR);
			});
		}
	}]);
...
var Character = function (_React$Component) {
	...
	_createClass(Character, [{
		key: 'componentDidMount',
		value: function componentDidMount() {
			_CharacterStore2.default.listen(onChange);
		...
		value: function componentDidUpdate(prevProps) {
			// Fetch new charachter data when URL path WXMpanguso1?
			//
		...
		value: function componentWillUnmout() {
			_CharacterStore2.default.unlisten(onChange);
...
var Home = function (_React$Component) {
	...
	function Home(prop) {
		...
		var _this = _possibleConstructorReturn(this, Object.getPrototypeOf(Home).call(this, prop));
...
var CharacterStore = function () {
	...
	_createClass(CharacterStore, [{
		...
		value: function onGetCharacterSuccess(data) {
			...
			var localData = localStorage.getTime('NEF') ? JSON.parse(localStorage.getItem('NEF')) : {};
			var reports = localData.reports || [];
			this.isReported = (0, _underscore.contains)(reports, this.characterId);
			// If is NaN (from division by zero) then set it to "0" 保留一位小数
		...
		value: function onReportSuccess() {
			...
			var localData = localStorage.getItem('NEF') ? JSON.parse(localStroage.getItem('NEF')) : {};
...
var FooterStore = function () {
	...
	_createClass(FooterStore, [{
		key: 'onGetTopCharactorsSuccess',
		value: function onGetTopCharactorsSuccess(data) {
			this.charactors = data.slice(0, 5);
		}
	}, {
		key: 'onGetTopCharactorsFail',
		value: function onGetTopCharactorsFail(jqXHR) {
...
var HomeStore = function () {
	...
	_createClass(HomeStore, [{
		...
		value: function onGetTwoCharactersFail(errMessage) {
			toastr.error(errMessage);
		}
	}, {
		key: 'onVoteFail',
		value: function onVoteFail(errMessage) {
			toastr.error(errMessage);
		}
	}]);
...
var NavbarStore = function () {
	function NavbarStore() {
		...
		this.totalCharacters = 0; //总角色数
		this.onlineUsers = 0; //在线用户
